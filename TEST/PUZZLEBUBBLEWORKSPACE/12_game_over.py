...
class Bubble(pygame.sprite.Sprite):
    ...
    def move(self):
        ...
        to_y = self.radius * math.sin(self.rad_angle) * -1 #y가 내려갈수록 + 올라갈수록 -
...
def get_bubble_position(row_idx, col_idx):
    ...
    pos_y = row_idx * CELL_SIZE + (BUBLLE_HEIGHT // 2) + wall_height
    if row_idx % 2 == 1:
        pos_x += CELL_SIZE // 2
...
def prepare_bubbles():
    ...
    if next_bubble:
        ...
        curr_bubble = create_bubbles() # 새 버블 만들기
...
def visit(row_idx,col_idx,color=None):
    ...
    if color and map[row_idx][col_idx] != color:
        return
    # 버블이 빈 공간이거나, 버블이 존재할 수 없는 위치인지 확인 
...
def draw_bubbles():
    ...
    if curr_fire_count == 2:
        to_x = random.randint(0, 2) -1 # -1 ~ 1 사이의 값이 나옴
    elif curr_fire_count == 1:
        to_x = random.randint(0, 8) -4 # -4 ~ 4 사이의 값
...
background = pygame.image.load(os.path.join(current_path,"background1.jpg"))
...
to_angle_left = 0 #왼쪽
to_angle_right = 0 #오른쪽
angle_speed = 1.5 # 1.5도씩 움직이게 됌.
